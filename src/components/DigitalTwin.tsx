...
function getAnimType(name: string, joints: string[]): string {
  ...
  if ((n.includes('finger') && !n.includes('wrist')) || j === 'fingers') return 'finger'
...
const PHASE_LABELS: Record<string, string[]> = {
  finger:    ['Hand Open', 'Index Moves In', 'Thumb Tap', 'Release Open'],
  fist:      ['Hand Open', 'Close Fist', 'Squeeze', 'Release Open'],
...
function MannequinSVG({ type, phase, live }: { type: string; phase: number; live?: DigitalTwinData }) {
  ...
  const isFingerTap = type === 'finger'
  const isFistStretch = type === 'fist'
  const handFocused = isFingerTap || isFistStretch

  // Compute angles from phase (0=start, 1=moving, 2=peak, 3=return)
  const rightShoulderAngle =
    type === 'shoulder' ? [0, -50, -155, -80][phase] ?? 0 :
    handFocused ? -38 : 0
  const rightElbowBend     =
    type === 'elbow' ? [0, 40, 90, 40][phase] ?? 0 :
    handFocused ? 76 : 0
  ...
  const rightWristAngle    =
    type === 'wrist' ? [0, 25, -25, 0][phase] ?? 0 :
    handFocused ? -12 : 0
  const rightAnkleAngle    = type === 'ankle'    ? [0, 20, -20, 10][phase]   ?? 0 : 0

  const tapProgress = isFingerTap ? ([0, 0.45, 1.0, 0.2][phase] ?? 0) : 0
  const fistProgress = isFistStretch ? ([0, 0.55, 1.0, 0.2][phase] ?? 0) : 0
  const indexTipX = 6 - tapProgress * 12 - fistProgress * 8
  const indexTipY = -22 + tapProgress * 18 + fistProgress * 14
  const middleTipY = -23 + fistProgress * 14
  const ringTipY = -21 + fistProgress * 13
  const pinkyTipY = -19 + fistProgress * 12
  const thumbTipX = -12 + fistProgress * 3
  const thumbTipY = 7 - fistProgress * 5
  ...
  return (
    <svg viewBox="0 0 220 320" width="100%" height="100%" style={{ maxHeight: 280 }}>
      ...
      <g transform={`rotate(${rightShoulderAngle}, 138, 72)`}>
        ...
        <g transform={`rotate(${rightElbowBend}, 162, 112)`}>
          ...
          <g transform={`rotate(${rightWristAngle}, 176, 150)`}>
            ...
            {isFistStretch && (
              <g transform="translate(181 162)">
                <ellipse cx="0" cy="0" rx="11" ry="14" fill={body} filter="url(#glow)" />
                <path d="M -7 1 C -8 -10, -2 -13, 1 -6" stroke={accent} strokeWidth="3" strokeLinecap="round" fill="none" />
                <path d="M -4 -1 C -5 -12, 1 -14, 4 -7" stroke={accent} strokeWidth="3" strokeLinecap="round" fill="none" />
                <path d="M -1 -2 C -2 -13, 4 -14, 7 -8" stroke={accent} strokeWidth="3" strokeLinecap="round" fill="none" />
                <path d="M 2 -1 C 2 -11, 8 -11, 10 -6" stroke={accent} strokeWidth="2.8" strokeLinecap="round" fill="none" />
                <path d="M -6 4 C -12 2, -13 8, -8 11" stroke={accent} strokeWidth="3" strokeLinecap="round" fill="none" />
                <circle cx="0" cy="0" r="2.5" fill={joint} />

                {phase === 1 && (
                  <circle cx="0" cy="0" r="17" fill="none" stroke={accent} strokeWidth="1.5" strokeDasharray="4,4" opacity="0.9" />
                )}
                {phase === 2 && (
                  <circle cx="0" cy="0" r="10" fill="#f9731640" stroke={accent} strokeWidth="1.3" />
                )}
              </g>
            )}
            ...
            {handFocused && (
              <g transform="translate(181 162)">
                <ellipse cx="0" cy="0" rx="10" ry="13" fill={body} filter="url(#glow)" />

                {/* Thumb */}
                <line x1="-2" y1="2" x2={thumbTipX} y2={thumbTipY} stroke={accent} strokeWidth="3.2" strokeLinecap="round" />

                {/* Index finger (animated tap) */}
                <line x1="3" y1="-8" x2={indexTipX} y2={indexTipY} stroke={accent} strokeWidth="3.3" strokeLinecap="round" />

                {/* Other fingers */}
                <line x1="1" y1="-8" x2="2" y2={middleTipY} stroke={body} strokeWidth="2.8" strokeLinecap="round" />
                <line x1="-1" y1="-6" x2="-1" y2={ringTipY} stroke={body} strokeWidth="2.6" strokeLinecap="round" />
                <line x1="-3" y1="-4" x2="-4" y2={pinkyTipY} stroke={body} strokeWidth="2.4" strokeLinecap="round" />

                <circle cx="0" cy="0" r="2.5" fill={joint} />

                {(isFingerTap && phase === 2) && (
                  <circle cx="-8" cy="2" r="4.5" fill="#f9731640" stroke={accent} strokeWidth="1.3" />
                )}
              </g>
            )}
          </g>
      ...
      {handFocused && (
        <>
          {/* Focus ring around hand */}
          <circle cx="188" cy="154" r="25" fill="none" stroke={accent} strokeWidth="1.5" strokeDasharray="4,4" opacity="0.9" />

          {/* Hand focus inset */}
          <g transform="translate(146 16)">
            <rect x="0" y="0" width="64" height="70" rx="10" fill="#0f1322dd" stroke="#f9731660" />
            <text x="8" y="11" fill="#f97316" fontSize="7" fontWeight="bold">
              {isFistStretch ? 'FIST FOCUS' : 'HAND FOCUS'}
            </text>

            <g transform="translate(33 44) scale(1.25)">
              {isFistStretch ? (
                <>
                  <ellipse cx="0" cy="0" rx="8" ry="10" fill={body} />
                  <path d="M -5 1 C -5 -6, -1 -8, 2 -4" stroke={accent} strokeWidth="2.4" strokeLinecap="round" fill="none" />
                  <path d="M -2 0 C -2 -7, 2 -8, 4 -4" stroke={accent} strokeWidth="2.4" strokeLinecap="round" fill="none" />
                  <path d="M 1 0 C 1 -7, 5 -7, 7 -3" stroke={accent} strokeWidth="2.2" strokeLinecap="round" fill="none" />
                  <path d="M -5 4 C -8 3, -8 7, -5 8" stroke={accent} strokeWidth="2.2" strokeLinecap="round" fill="none" />
                </>
              ) : (
                <>
                  <ellipse cx="0" cy="0" rx="8" ry="10" fill={body} />
                  <line x1="-1" y1="2" x2={thumbTipX * 0.8} y2={thumbTipY * 0.8} stroke={accent} strokeWidth="2.5" strokeLinecap="round" />
                  <line x1="2" y1="-6" x2={indexTipX * 0.9} y2={indexTipY * 0.9} stroke={accent} strokeWidth="2.5" strokeLinecap="round" />
                  <line x1="0" y1="-6" x2="1" y2={middleTipY * 0.9} stroke={body} strokeWidth="2.1" strokeLinecap="round" />
                  <line x1="-2" y1="-5" x2="-2" y2={ringTipY * 0.9} stroke={body} strokeWidth="1.9" strokeLinecap="round" />
                  <line x1="-3" y1="-3" x2="-4" y2={pinkyTipY * 0.9} stroke={body} strokeWidth="1.7" strokeLinecap="round" />
                  {(isFingerTap && phase === 2) && (
                    <circle cx="-7" cy="2" r="4" fill="#f9731630" stroke={accent} strokeWidth="1" />
                  )}
                </>
              )}
            </g>
...
export default function DigitalTwin({
  ...
  useEffect(() => {
    ...
    if (!isActive) return
    const phaseMs = type === 'finger' ? 1000 : 1400
    ...
    <div className="glass-card overflow-hidden flex flex-col">
      ...
      <div className="px-4 py-3 border-t border-border space-y-2.5 bg-page">
        ...
        <div className="flex justify-between text-xs text-text-muted">
          ...
          <span className="font-semibold text-text-primary">{targetRomMin}°–{targetRomMax}°</span>
        </div>
        <div className="flex justify-between text-xs text-text-muted">
